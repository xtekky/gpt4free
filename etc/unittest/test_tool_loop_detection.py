...
class TestToolLoopDetection(unittest.TestCase):
    ...
    def test_two_calls_allowed(self):
        ...
        messages = _make_loop(
            "file_search", '{"query": "**/x.js"}', "No files found", 2
        )
        detect_tool_loop(messages)  # should not raise
    ...
    def test_three_identical_calls_blocked(self):
        ...
        messages = _make_loop(
            "file_search", '{"query": "**/members-worker.js"}', "No files found", 3
        )
    ...
    def test_different_arguments_not_blocked(self):
        ...
        for i in range(5):
            ...
            messages.append(
                {"role": "tool", "tool_call_id": f"call_{i}", "content": "(empty)"}
            )
    ...
    def test_non_empty_result_not_blocked(self):
        ...
        messages = _make_loop(
            "file_search", '{"query": "x"}', "Found 3 files: a, b, c", 5
        )
