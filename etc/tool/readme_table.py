    ...
    try:
        response = await asyncio.wait_for(
            ChatCompletion.create_async(
                model=models.default, messages=messages, provider=provider
            ),
            30,
        )
...
def test_async_list(providers: list[ProviderType]):
    responses: list = [asyncio.run(test_async(_provider)) for _provider in providers]
    return responses
            ...
            if issubclass(_provider, ProviderModelMixin):
                try:
                    ...
                    models = [
                        ...
                        if model in all_models or model in _provider.model_aliases
                    ]
                    ...
                    if image_models:
                        ...
                        image_models = [
                            model for model in image_models if model in _all_models
                        ]
                        if image_models:
                            models = [
                                model for model in models if model not in image_models
                            ]
                    if models:
                        lines.append(
                            f"| **Models** | {', '.join(models)} ({len(all_models)})|"
                        )
                    if image_models:
                        lines.append(
                            f"| **Image Models (Image Generation)** | {', '.join(image_models)} |"
                        )
...
def print_image_models():
    ...
    for provider in [
        ...
        if provider.working
        and getattr(provider, "image_models", None)
        or getattr(provider, "vision_models", None)
    ]:
        ...
        if provider.image_models:
            image_models = ", ".join(
                [model for model in provider.image_models if model in _all_models]
            )
        else:
            image_models = "❌"
        if hasattr(provider, "vision_models"):
            ...
            vision_models = "❌"
        lines.append(
            f"| {label} | `g4f.Provider.{provider.__name__}` | {image_models}| {vision_models} | {website} |"
        )
