...
def stream_read_files(
    bucket_dir: Path, filenames: list[str], delete_files: bool = False
) -> Iterator[str]:
    for filename in filenames:
        ...
        if filename.endswith(".zip"):
            with zipfile.ZipFile(file_path, "r") as zip_ref:
                ...
                try:
                    yield from stream_read_files(
                        bucket_dir,
                        [f for f in zip_ref.namelist() if supports_filename(f)],
                        delete_files,
                    )
...
def stream_read_parts_and_refine(
    bucket_dir: Path, delete_files: bool = False
) -> Iterator[str]:
    cache_file = bucket_dir / PLAIN_CACHE
...
def split_file_by_size_and_newline(
    input_filename, output_dir, chunk_size_bytes=1024 * 1024
):  # 1MB
    ...
    with open(input_filename, "r", encoding="utf-8") as infile:
        ...
        for line in infile:
            ...
            if current_chunk_size >= chunk_size_bytes:
                if (
                    is_complete(current_chunk)
                    or current_chunk_size >= chunk_size_bytes * 2
                ):
                    output_filename = (
                        f"{output_prefix}{chunk_num:04d}{split_filename[1]}"
                    )
...
async def download_urls(
    ...
    async with ClientSession(
        connector=get_connector(proxy=proxy), timeout=ClientTimeout(timeout)
    ) as session:
            ...
            try:
                async with session.get(url) as response:
                    ...
                    if not filename:
                        ...
                        return None
                    if (
                        not is_allowed_extension(filename)
                        and not supports_filename(filename)
                        or filename == DOWNLOADS_FILE
                    ):
                        return None
                    ...
                    with target.open("wb") as f:
                        async for chunk in response.content.iter_any():
                            if (
                                ...
                                and b'<link rel="canonical"' not in chunk
                            ):
                                f.write(
                                    chunk.replace(
                                        b"</head>",
                                        f'<link rel="canonical" href="{response.url}">\n</head>'.encode(),
                                    )
                                )
                            else:
                                f.write(chunk)
        ...
        for filename in await asyncio.gather(
            *[download_url(url, max_depth) for url in urls]
        ):
            if filename:
                yield filename
            else:
        ...
        while new_urls:
            ...
            for i in range(0, len(new_urls), group_size):
                ...
                async for filename in download_urls(
                    bucket_dir,
                    chunked_urls,
                    max_depth - 1,
                    loading_urls,
...
def read_and_download_urls(
    bucket_dir: Path, delete_files: bool = False, event_stream: bool = False
) -> Iterator[str]:
...
async def async_read_and_download_urls(
    bucket_dir: Path, delete_files: bool = False, event_stream: bool = False
) -> AsyncIterator[str]:
    urls = get_downloads_urls(bucket_dir, delete_files)
    if urls:
...
def stream_chunks(
    ...
    refine_chunks_with_spacy: bool = False,
    ...
    if refine_chunks_with_spacy:
        ...
        streaming = stream_read_files(
            bucket_dir, get_filenames(bucket_dir), delete_files
        )
...
async def get_async_streaming(
    ...
    bucket_dir.mkdir(parents=True, exist_ok=True)
    try:
        async for chunk in async_read_and_download_urls(
            bucket_dir, delete_files, event_stream
        ):
            yield chunk
