...
class HuggingChat(AsyncAuthedProvider, ProviderModelMixin):
    ...
    def get_models(cls, **kwargs) -> list[str]:
        if not cls.models:
            try:
                models = (
                    requests.get(
                        f"{cls.url}/api/v2/models", timeout=kwargs.get("timeout", 15)
                    )
                    .json()
                    .get("json")
                )
                ...
                debug.error(
                    f"{cls.__name__}: Error reading models: {type(e).__name__}: {e}"
                )
        return cls.models
    ...
    async def on_auth_async(
        ...
        if cookies is None:
            cookies = get_cookies(
                cls.domain, raise_requirements_error=False, single_browser=True
            )
        try:
            yield RequestLogin(cls.__name__, os.environ.get("G4F_LOGIN_URL") or "")
            yield AuthResult(
                **await get_args_from_nodriver(
                    cls.url, proxy=proxy, wait_for="div div nav div div img"
                )
            )
