    ...
    async def create_async_generator(
        ...
        **kwargs,
        ...
        if model != cls.default_image_model and model in cls.image_models:
            image_size = model
        yield await cls.generate(
            format_media_prompt(messages, prompt), image_size, proxy
        )

    @classmethod
    async def generate(
        cls, prompt: str, image_size: str, proxy: str = None
    ) -> ImageResponse:
        try:
            access_token, user_agent = readHAR(
        ...
        images = await create_images(
            prompt, access_token, user_agent, image_size, proxy
        )
...
async def create_images(
    ...
    async with aiohttp.ClientSession(connector=get_connector(proxy=proxy)) as session:
        ...
        form_data.add_field(
            "dalle-boost-count", response_data.get("dalle-boost-count", 0)
        )
        polling_meta_data = response_data.get("polling_response", {}).get(
            "polling_meta_data", {}
        )
        ...
        while True:
            ...
            images = [
                image["ImageUrl"]
                for image in response_data.get("image_urls_thumbnail", [])
            ]
            if images:
                return images
