...
class YouTube(AsyncGeneratorProvider, ProviderModelMixin):
    ...
    async def create_async_generator(
        ...
        results = [
            {"id": line.split("?v=")[-1].split("&")[0], "url": line}
        ...
        for result in results:
            ...
            for message in messages:
                if isinstance(message.get("content"), str):
                    if video_url in message["content"] and (
                        model == "search" or model in message["content"]
                    ):
                        has_video = True
                        break
            if has_video:
        ...
        if model == "search":
            ...
            if new_results:
                ...
                path = await provider.download(
                    video_url, model=model, output_dir=get_media_dir()
                )
...
class YouTubeProvider:
    ...
    async def download(
        self, video_url: str, model: str = "720p", output_dir: str = "."
    ) -> str:
        """
